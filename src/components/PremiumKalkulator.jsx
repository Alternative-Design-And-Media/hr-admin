...
const hm = (m) => { if (!m) return "0:00"; const h=Math.floor(Math.abs(m)/60),mm=Math.round(Math.abs(m)%60); return (m<0?"-":"")+h+":"+(mm<10?"0":"")+mm; };
...
function Num({value,onChange,min,step,style}){
  ...
  const st=style||{};
  return(<input type="number" value={value} min={min!=null?min:undefined} step={s} onChange={e=>onChange(Number(e.target.value))} style={{width:58,background:C.surf,border:"1px solid "+C.border,color:C.text,borderRadius:5,padding:"3px 6px",fontSize:12,textAlign:"right",...st}}/>);
}
...
export default function PremiumKalkulator(){
  ...
  const calcRows=()=>{
    if(!result) return [];
    return Object.entries(result.matched).map(([planName,matched])=>{
      const plan=matched.plan; const actKey=matched.actKey; const actData=matched.actData;
  ...
  const generatePrintHTML=()=>{
    ...
    return "<!DOCTYPE html><html lang='hu'><head><meta charset='UTF-8'/><title>Prémium – "+employee+" – "+mLabel+"</title><style>"+css+"</style></head><body>"
      ...
      +"<h2>Kalkuláció</h2><div class='calc-box'>Alapdíj: "+fmtHu(BASE_RATE)+" Ft/h<br>Kompetencia: "+kompScore+"% x 10 Ft = +"+(kompScore*10)+" Ft/h<br>Számított óradíj: "+fmtHu(kompRate)+" Ft/h<br><br>Rentman: "+hm(rentmanPayableMin)+" | Egyéb: "+hm(manualPayableMin)+" | Összes: "+hm(totalPayableMin)+" = "+(totalPayableMin/60).toFixed(4)+" h<br><br><strong>Prémium = "+(totalPayableMin/60).toFixed(4)+" h x "+fmtHu(kompRate)+" Ft/h = "+fmtHu(premium)+" Ft</strong></div>"
      ...
      <div style={{background:C.card,border:brd(C.border),borderRadius:12,overflow:"hidden"}}>
        ...
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,alignItems:"end"}}>
            <div><div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:5}}>Munkavállaló neve</div><input value={employee} onChange={e=>setEmployee(e.target.value)} style={{width:"100%",boxSizing:"border-box",background:C.surf,border:brd(C.border),color:C.text,borderRadius:6,padding:"8px 11px",fontSize:13}}/></div>
            <div><div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:5}}>Hónap</div><input type="month" value={month} onChange={e=>setMonth(e.target.value)} style={{width:"100%",boxSizing:"border-box",background:C.surf,border:brd(C.border),color:C.text,borderRadius:6,padding:"8px 11px",fontSize:13}}/></div>
      ...
      {result&&<>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[{l:"Rentman elszámolt",v:hm(rentmanPayableMin),u:"óra",hi:false,big:false,dim:false},{l:"Manuális prémiumóra",v:hm(manualPayableMin),u:"óra",dim:true,hi:false,big:false},{l:"Összes prémiumóra",v:hm(totalPayableMin),u:"óra",hi:true,big:false,dim:false},{l:"Fizetendő prémium",v:fmtHu(premium),u:"Ft bruttó",big:true,hi:false,dim:false}].map(function(item){
            const l=item.l,v=item.v,u=item.u,hi=item.hi,big=item.big,dim=item.dim;
            return <div key={l} style={{background:big?C.accentDim:C.card,border:brd(big?C.accent:C.border),borderRadius:10,padding:"13px 15px"}}>
        ...
        <div style={{background:C.card,border:brd(C.border),borderRadius:12,overflow:"hidden"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:C.thead}}>{["Projekt","Terv","NORMA rendez.","Túlóra (+) h","Kiesés (−) h","Kiesés oka","Elszámolt","Státusz"].map(h=>(<th key={h} style={{padding:"8px 11px",textAlign:h==="Projekt"||h.includes("oka")?"left":"right",color:C.muted,fontSize:10,fontWeight:700,letterSpacing:.4,textTransform:"uppercase",borderBottom:brd(C.border)}}>{h}</th>))}</tr></thead>
            <tbody>
              {rows.map((r,i)=>(
                <tr key={i} style={{borderBottom:"1px solid "+C.border+"22",background:i%2?"transparent":C.surf+"33"}}>
                  ...
                  <td style={{padding:"9px 11px"}}><input value={corrections[r.planName]&&corrections[r.planName].absenceReason||""} onChange={e=>setCorr(r.planName,"absenceReason",e.target.value)} placeholder="betegség, szabadság…" style={{width:"100%",boxSizing:"border-box",background:C.surf,border:brd(C.border),color:C.text,borderRadius:5,padding:"3px 7px",fontSize:11}}/></td>
          ...
          <div style={{padding:"10px 14px",borderTop:brd(C.border),display:"flex",flexDirection:"column",gap:6}}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:C.muted,minWidth:200,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.planName}:</span><input value={corrections[r.planName]&&corrections[r.planName].note||""} onChange={e=>setCorr(r.planName,"note",e.target.value)} placeholder="megjegyzés az elszámoláshoz…" style={{flex:1,background:C.surf,border:brd(C.border),color:C.text,borderRadius:5,padding:"3px 8px",fontSize:11}}/></div>))}
          </div>
        </div>
        {manualAllocs.length>0&&(
          <div style={{background:C.card,border:brd(C.border),borderRadius:12,overflow:"hidden"}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <thead><tr style={{background:C.thead}}>{["NORMA projekt","NORMA összes","Rendez.techn.","Altípusok","Elfogadott h","Jogcím / Megjegyzés"].map(h=>(<th key={h} style={{padding:"8px 11px",textAlign:h.includes("projekt")||h.includes("Altípus")||h.includes("Jogcím")?"left":"right",color:C.muted,fontSize:10,fontWeight:700,letterSpacing:.4,borderBottom:brd(C.border)}}>{h}</th>))}</tr></thead>
              <tbody>
                {manualAllocs.map((a,i)=>(<tr key={i} style={{borderBottom:"1px solid "+C.border+"22",background:i%2?"transparent":C.surf+"33"}}>
                  ...
                  <td style={{padding:"9px 11px"}}><input value={a.note||""} onChange={e=>setManualAllocs(p=>p.map((x,j)=>j===i?{...x,note:e.target.value}:x))} placeholder="egyedi megbízás, tulajdonosi döntés…" style={{width:"100%",boxSizing:"border-box",background:C.surf,border:brd(C.border),color:C.text,borderRadius:5,padding:"3px 8px",fontSize:11}}/></td>
                </tr>))}
        ...
        {result.noProject.length>0&&(
          <div style={{background:C.card,border:brd(C.border),borderRadius:12,overflow:"hidden"}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <thead><tr style={{background:C.thead}}>{["Dátum","Típus","Idő","Megjegyzés"].map(h=>(<th key={h} style={{padding:"7px 11px",textAlign:"left",color:C.muted,fontSize:10,fontWeight:700,borderBottom:brd(C.border)}}>{h}</th>))}</tr></thead>
        ...
        <div style={{background:C.card,border:"1px solid "+C.accent+"44",borderRadius:12,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:14}}>
            ...
            <div style={{display:"flex",gap:8,flexShrink:0}}>
              ...
              <button onClick={showMD} style={{padding:"9px 16px",borderRadius:7,border:brd(C.border),background:C.surf,color:C.muted,fontWeight:600,fontSize:12,cursor:"pointer"}}>{mdVisible?"↻ MD frissítés":"MD forrás"}</button>
              {mdVisible&&<button onClick={copyMD} style={{padding:"9px 16px",borderRadius:7,border:brd(copied?C.ok:C.border),background:copied?"#052e16":C.surf,color:copied?C.ok:C.text,fontWeight:700,fontSize:13,cursor:"pointer",transition:"all .2s"}}>{copied?"✓ Másolva!":"📋 Másolás"}</button>}
            </div>
          </div>
          {mdVisible&&(
            <div style={{borderTop:brd(C.border),padding:14}}>
              ...
              <textarea readOnly value={mdText} style={{width:"100%",boxSizing:"border-box",height:280,background:C.bg,border:brd(C.border),color:"#94a3b8",fontFamily:"'Courier New',monospace",fontSize:11,lineHeight:1.6,borderRadius:7,padding:12,resize:"vertical"}} onFocus={e=>e.target.select()}/>
            </div>
          )}
        </div>
      </>}
