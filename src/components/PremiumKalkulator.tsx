...
const hm = (m: number) => { if (!m) return "0:00"; const h=Math.floor(Math.abs(m)/60),mm=Math.round(Math.abs(m)%60); return `${m<0?"-":""}${h}:${String(mm).padStart(2,"0")}`; };
...
export default function PremiumKalkulator(){
  ...
  const generateMD=()=>{
    ...
    lines.push(``); lines.push(`---`); lines.push(`## Összefoglaló`); lines.push(``);
    ...
    lines.push(``); lines.push(`---`); lines.push(`## 1. Rentman projektek részletezése`); lines.push(``);
    lines.push(`> Számítási alap: RENTMAN tervezett = kifizethető maximum.`); lines.push(``);
    ...
    lines.push(``);
    if(rows.some(r=>r.note)){ lines.push(`### Megjegyzések`); for(const r of rows.filter(r=>r.note)) lines.push(`- **${r.planName}:** ${r.note}`); lines.push(``); }
    if(manualAllocs.some(a=>a.allocH>0)){
      lines.push(`---`); lines.push(`## 2. Manuálisan elfogadott egyéb prémiumórák`); lines.push(``);
      ...
      lines.push(`| **ÖSSZESEN** | | **${hm(manualPayableMin)}** | |`); lines.push(``);
    }
    if(result?.unmatchedAct?.some(u=>!manualAllocs.find(a=>a.actKey===u.actKey&&a.allocH>0))){
      lines.push(`---`); lines.push(`## 3. Nem elszámolt NORMA projektek`); lines.push(``);
      lines.push(`| Projekt | Összes | Rendez.techn. | Altípusok |`);
      lines.push(`|---|---:|---:|---|`);
      for(const u of result.unmatchedAct){ const a=manualAllocs.find(x=>x.actKey===u.actKey); if(a?.allocH>0) continue; lines.push(`| ${u.actKey} | ${hm(u.actData.totalMin)} | ${hm(u.actData.rendezMin)} | ${Object.entries(u.actData.byType).map(([t,m])=>`${t}: ${hm(m)}`).join(", ")} |`); }
      lines.push(``);
    }
    if(result?.noProject?.length){
      lines.push(`---`); lines.push(`## 4. Projekt nélküli leadott órák`); lines.push(``);
      lines.push(`| Dátum | Típus | Idő | Megjegyzés |`);
      lines.push(`|---|---|---:|---|`);
      for(const r of result.noProject){ const d=r.date instanceof Date?r.date.toLocaleDateString("hu-HU"):String(r.date).slice(0,10); lines.push(`| ${d} | ${r.altipus} | ${hm(r.mins)} | ${r.note||"—"} |`); }
      lines.push(``);
    }
    lines.push(`---`); lines.push(`## Kalkuláció`); lines.push(``);
    lines.push("```");
    ...
    lines.push("```");
    lines.push(``);
    lines.push(`---`); lines.push(`## Jóváhagyás`); lines.push(``);
        ...
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,alignItems:"end"}}>
            <div>
              ...
              <input value={employee} onChange={e=>setEmployee(e.target.value)} style={{width:"100%",boxSizing:"border-box",background:C.surf,border:`1px solid ${C.border}`,color:C.text,borderRadius:6,padding:"8px 11px",fontSize:13}}/>
            </div>
            <div>
              <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:5}}>Hónap</div>
              <input type="month" value={month} onChange={e=>setMonth(e.target.value)} style={{width:"100%",boxSizing:"border-box",background:C.surf,border:`1px solid ${C.border}`,color:C.text,borderRadius:6,padding:"8px 11px",fontSize:13}}/>
            </div>
            <div>
              <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:5}}>Kompetencia: <span style={{color:C.accent}}>{kompScore}%</span><span style={{color:C.muted,fontWeight:400}}> → {fmtHu(kompRate)} Ft/h</span></div>
          ...
          <div style={{padding:"10px 14px",borderTop:`1px solid ${C.border}`,display:"flex",flexDirection:"column",gap:6}}>
            ...
            {rows.map((r,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <input value={corrections[r.planName]?.note||""} onChange={e=>setCorr(r.planName,"note",e.target.value)} placeholder="megjegyzés az elszámoláshoz…" style={{flex:1,background:C.surf,border:`1px solid ${C.border}`,color:C.text,borderRadius:5,padding:"3px 8px",fontSize:11}}/>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              ...
              <tbody>
                ...
                <tr style={{background:C.thead,borderTop:`2px solid ${C.border}`}}>
                  <td style={{padding:"9px 11px",fontWeight:700}}>Összesen</td><td colSpan={3}/>
                  <td style={{padding:"9px 11px",textAlign:"right",fontFamily:"monospace",fontWeight:800,color:manualPayableMin>0?C.ok:C.muted}}>{hm(manualPayableMin)}</td><td/>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div style={{background:C.card,border:`1px solid ${C.accent}44`,borderRadius:12,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:14}}>
            ...
            <div style={{display:"flex",gap:8,flexShrink:0}}>
              <button onClick={showMD} style={{padding:"9px 18px",borderRadius:7,border:`1px solid ${C.accent}`,background:C.accentDim,color:C.accent,fontWeight:700,fontSize:13,cursor:"pointer"}}>{mdVisible?"↻ Frissítés":"Jelentés generálása →"}</button>
              {mdVisible&&<button onClick={copyMD} style={{padding:"9px 18px",borderRadius:7,border:`1px solid ${copied?C.ok:C.border}`,background:copied?"#052e16":C.surf,color:copied?C.ok:C.text,fontWeight:700,fontSize:13,cursor:"pointer",transition:"all .2s"}}>{copied?"✓ Másolva!":"📋 Másolás"}</button>}
            </div>
          </div>
          {mdVisible&&(
            <div style={{borderTop:`1px solid ${C.border}`,padding:14}}>
              <div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:.8,textTransform:"uppercase",marginBottom:6}}>Markdown tartalom — jelöld ki az összeset (Ctrl+A) majd másold (Ctrl+C)</div>
              <textarea readOnly value={mdText} style={{width:"100%",boxSizing:"border-box",height:320,background:C.bg,border:`1px solid ${C.border}`,color:"#94a3b8",fontFamily:"'Courier New',monospace",fontSize:11,lineHeight:1.6,borderRadius:7,padding:12,resize:"vertical"}} onFocus={e=>e.target.select()}/>
            </div>
          )}
        </div>
      </>}
