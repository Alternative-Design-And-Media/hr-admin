...
function Num({value,onChange,min,step=1,style={}}){
  return(<input type="number" value={value} min={min!=null?min:undefined} step={step} onChange={e=>onChange(Number(e.target.value))} style={{width:58,background:C.surf,border:`1px solid ${C.border}`,color:C.text,borderRadius:5,padding:"3px 6px",fontSize:12,textAlign:"right",...style}}/>);
}
...
export default function PremiumKalkulator(){
  ...
  const generatePrintHTML=()=>{
    ...
    const tbl=(headers,body)=>"<table><thead><tr>"+headers.map(h=>"<th>"+h+"</th>").join("")+"</tr></thead><tbody>"+body+"</tbody></table>";
    const rentmanRows=rows.map(r=>"<tr><td>"+r.planName+(r.actKey&&r.actKey!==r.planName?"<br><small>↳ "+r.actKey+"</small>":"")+"</td><td class='num'>"+hm(r.plannedMin)+"</td><td class='num'>"+hm(r.actRendezMin)+"</td><td class='num'>"+(r.overtimeMin?"+"+hm(r.overtimeMin):"—")+"</td><td class='num'>"+(r.absenceMin?"-"+hm(r.absenceMin)+(r.absenceReason?" ("+r.absenceReason+")"):"—")+"</td><td class='num bold "+(r.payableMin>0?"ok":"muted")+"'>"+hm(r.payableMin)+"</td><td><span class='badge "+r.flagLevel+"'>"+(r.flagLevel==="ok"?"✓ OK":r.flagLevel==="rev"?"⚑ Jóváhagyás":r.flagLevel==="info"?"ℹ Tény>Terv":"⚠ Hiányzó")+"</span></td></tr>").join("")+"<tr class='total'><td>Összesen</td><td class='num'>"+hm(rows.reduce((s,r)=>s+r.plannedMin,0))+"</td><td class='num'>"+hm(rows.reduce((s,r)=>s+r.actRendezMin,0))+"</td><td></td><td></td><td class='num bold ok'>"+hm(rentmanPayableMin)+"</td><td></td></tr>";
    ...
    return "<!DOCTYPE html><html lang='hu'><head><meta charset='UTF-8'/><title>Prémium Elszámolás – "+employee+" – "+mLabel+"</title><style>@page{size:A4;margin:18mm 15mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;font-size:10pt;color:#1a1a2e;line-height:1.4}.header{border-bottom:3px solid #4f7cff;padding-bottom:10px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:flex-end}.header-left h1{font-size:16pt;color:#4f7cff;margin-bottom:2px}.header-left p{font-size:9pt;color:#555}.header-right{text-align:right;font-size:9pt;color:#555}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px}.summary-card{border:1.5px solid #e0e4f0;border-radius:6px;padding:8px 10px}.summary-card .label{font-size:7.5pt;color:#888;text-transform:uppercase;letter-spacing:.4px;margin-bottom:3px}.summary-card .value{font-size:13pt;font-weight:800;color:#1a1a2e}.summary-card .value.accent{color:#4f7cff}.summary-card .value.ok{color:#16a34a}.summary-card .unit{font-size:8pt;color:#888}.summary-card.highlight{border-color:#4f7cff;background:#f0f4ff}h2{font-size:11pt;color:#4f7cff;margin:14px 0 4px;border-bottom:1px solid #dde3f5;padding-bottom:3px}h3{font-size:10pt;color:#333;margin:10px 0 4px}p.sub{font-size:8.5pt;color:#777;margin-bottom:6px;font-style:italic}table{width:100%;border-collapse:collapse;font-size:8.5pt;margin-bottom:10px;page-break-inside:avoid}th{background:#f0f4ff;color:#4f7cff;text-align:right;padding:5px 7px;font-size:7.5pt;text-transform:uppercase;letter-spacing:.3px;border-bottom:1.5px solid #c7d2f0}th:first-child{text-align:left}td{padding:5px 7px;border-bottom:1px solid #eef0f8;vertical-align:top}td:first-child{text-align:left}td.num{text-align:right;font-family:'Courier New',monospace}td.bold{font-weight:700}td.ok{color:#16a34a}td.muted{color:#aaa}td.small{font-size:7.5pt;color:#888}tr.total td{background:#eef1fc!important;font-weight:700;border-top:1.5px solid #c7d2f0}tr:nth-child(even) td{background:#fafbff}.badge{display:inline-block;border-radius:3px;padding:1px 5px;font-size:7.5pt;font-weight:700}.badge.ok{background:#dcfce7;color:#166534}.badge.warn{background:#fef9c3;color:#854d0e}.badge.rev{background:#fee2e2;color:#991b1b}.badge.info{background:#dbeafe;color:#1e40af}small{font-size:7.5pt;color:#888}ul{margin:0 0 8px 16px}li{font-size:9pt;margin-bottom:2px}.calc-box{background:#f7f9ff;border:1px solid #dde3f5;border-radius:5px;padding:10px 14px;font-family:'Courier New',monospace;font-size:8.5pt;line-height:1.8;margin:8px 0 14px}.approval{margin-top:20px;page-break-inside:avoid}.approval table td{padding:10px 7px;font-size:9pt}.footer{margin-top:16px;padding-top:8px;border-top:1px solid #dde3f5;font-size:7.5pt;color:#aaa;font-style:italic}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>"
      ...
      +"<div class='summary'><div class='summary-card'><div class='label'>Rentman elszámolt</div><div class='value ok'>"+hm(rentmanPayableMin)+"</div><div class='unit'>óra</div></div><div class='summary-card'><div class='label'>Manuális prémiumóra</div><div class='value'>"+hm(manualPayableMin)+"</div><div class='unit'>óra</div></div><div class='summary-card'><div class='label'>Összes prémiumóra</div><div class='value ok'>"+hm(totalPayableMin)+"</div><div class='unit'>óra</div></div><div class='summary-card highlight'><div class='label'>Fizetendő prémium</div><div class='value accent'>"+fmtHu(premium)+"</div><div class='unit'>Ft bruttó</div></div></div>"
      ...
      +"<h2>Kalkuláció</h2><div class='calc-box'>Alapdíj: "+fmtHu(BASE_RATE)+" Ft/h<br>Kompetencia pontszám: "+kompScore+"% × 10 Ft = +"+kompScore*10+" Ft/h<br>Számított óradíj: "+fmtHu(kompRate)+" Ft/h<br><br>Elszámolt óra (Rentman): "+hm(rentmanPayableMin)+"<br>Elszámolt óra (egyéb): "+hm(manualPayableMin)+"<br>Összes prémiumóra: "+hm(totalPayableMin)+" = "+(totalPayableMin/60).toFixed(4)+" h<br><br><strong>Prémium = "+(totalPayableMin/60).toFixed(4)+" h × "+fmtHu(kompRate)+" Ft/h = "+fmtHu(premium)+" Ft</strong></div>"
      +"<div class='approval'><h2>Jóváhagyás</h2><table><thead><tr><th style='text-align:left'>Szerepkör</th><th>Dátum</th><th>Aláírás</th></tr></thead><tbody><tr><td>Munkavállaló</td><td class='num'>_________________</td><td class='num'>_________________</td></tr><tr><td>Projektvezető</td><td class='num'>_________________</td><td class='num'>_________________</td></tr><tr><td>Bérszámfejtő</td><td class='num'>_________________</td><td class='num'>_________________</td></tr></tbody></table></div>"
      ...
      {result&&<>
        ...
        <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:12,overflow:"hidden"}}>
            ...
            <tbody>
              {rows.map((r,i)=>(
                <tr key={i} style={{borderBottom:"1px solid "+C.border+"22",background:i%2?"transparent":C.surf+"33"}}>
                  ...
                  <td style={{padding:"9px 11px",textAlign:"right"}}><Num value={corrections[r.planName]&&corrections[r.planName].overtimeH||0} min={0} step={0.5} onChange={v=>setCorr(r.planName,"overtimeH",v)}/></td>
                  <td style={{padding:"9px 11px",textAlign:"right"}}><Num value={corrections[r.planName]&&corrections[r.planName].absenceH||0} min={0} step={0.5} onChange={v=>setCorr(r.planName,"absenceH",v)}/></td>
                  <td style={{padding:"9px 11px"}}><input value={corrections[r.planName]&&corrections[r.planName].absenceReason||""} onChange={e=>setCorr(r.planName,"absenceReason",e.target.value)} placeholder="betegség, szabadság…" style={{width:"100%",boxSizing:"border-box",background:C.surf,border:"1px solid "+C.border,color:C.text,borderRadius:5,padding:"3px 7px",fontSize:11}}/></td>
          ...
          <div style={{padding:"10px 14px",borderTop:"1px solid "+C.border,display:"flex",flexDirection:"column",gap:6}}>
            ...
            {rows.map((r,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:C.muted,minWidth:200,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.planName}:</span><input value={corrections[r.planName]&&corrections[r.planName].note||""} onChange={e=>setCorr(r.planName,"note",e.target.value)} placeholder="megjegyzés az elszámoláshoz…" style={{flex:1,background:C.surf,border:"1px solid "+C.border,color:C.text,borderRadius:5,padding:"3px 8px",fontSize:11}}/></div>))}
          </div>
        </div>
